...
It does not (yet) decode plugin state or the mixer graph - those events pass through writes untouched, but `read()` does not expose them. Playlist reading landed in 0.2.0 and automation-clip reading in 0.3.0; writers for both have not.
